...
function VendorForm({ defaultValues, onSubmit, submitting }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-sm font-medium text-slate-700">Opening Balance (NGN)</label>
          <input
            type="number"
            step="0.01"
            className="mt-1 w-full rounded border border-slate-300 px-3 py-2"
          ...
          />
          <p className="mt-0.5 text-xs text-slate-400">Amount owed to vendor before go-live</p>
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700">As of Date</label>
          <input
            type="date"
            className="mt-1 w-full rounded border border-slate-300 px-3 py-2"
...
export function VendorsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Vendors"
        action={canManage && <Button onClick={() => setModal("create")}>Add Vendor</Button>}
      />
      <input
      ...
      />
      <DataTable columns={columns} rows={data?.items} loading={isLoading} emptyMessage="No vendors yet." />

      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === "create" ? "Add Vendor" : "Edit Vendor"}>
        <VendorForm
