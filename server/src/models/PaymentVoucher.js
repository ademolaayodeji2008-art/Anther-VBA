...
const approvalHistorySchema = new Schema(
  {
    action: {
      type: String,
      enum: ["RAISED", "APPROVED", "REJECTED", "PAID", "RESUBMITTED"],
      required: true,
    },
...
const paymentVoucherSchema = new Schema(
  {
    ...
    // Replaces the VBA's four parallel sheets (Pending/Approved/Rejected/TreatedPayables) with
    // one append-only audit trail on the voucher itself.
...
export default model("PaymentVoucher", paymentVoucherSchema);
