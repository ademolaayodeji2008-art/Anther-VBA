...
import { optionalDate, requiredDate } from "../utils/zodHelpers.js";
...
export async function listExpenses(req, res, next) {
  try {
    ...
    if (startDate || endDate) { const { start, end } = parseDateRange(req.query); filter.date = { $gte: start, $lte: end }; }
    if (search) Object.assign(filter, buildSearchFilter(["particulars","expenseNo","category","remarks"], search));
    const [items, total] = await Promise.all([
      Expense.find(filter).populate("bank","name").sort({ date: -1 }).skip(skip).limit(limit),
...
export async function createExpenseHandler(req, res, next) {
  try {
    const expense = await createExpense(req.tenantDb, req.body, { postedBy: req.user.id });
...
export async function updateExpenseHandler(req, res, next) {
  try {
    const expense = await updateExpense(req.tenantDb, req.params.id, req.body);
    res.json(expense);
  } catch (err) { next(err); }
}
...
export async function deleteExpenseHandler(req, res, next) {
  try {
    const result = await deleteExpense(req.tenantDb, req.params.id);
    res.json(result);
  } catch (err) { next(err); }
}
