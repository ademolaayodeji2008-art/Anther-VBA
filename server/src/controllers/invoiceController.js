...
const lineSchema = z
  ...
  .refine((data) => data.saleType !== "BUNDLE" || !!data.converter, {
    message: "converter (quantity per bundle) is required when sale type is Bundle",
    path: ["converter"],
  });
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      Invoice.find(filter)
        ...
        .populate("bank", "name accountNo accountName")
        .sort({ issueDate: -1 })
        .skip(skip)
        .limit(limit),
...
export async function createInvoiceHandler(req, res, next) {
  try {
    const invoice = await createInvoice(req.body, { postedBy: req.user.id });
