...
const lineSchema = z.object({ item: z.string().min(1), qty: z.number().positive(), unitPrice: z.number().min(0) });
...
}).refine((d) => d.paymentType !== "BANK" || !!d.bank, { message: "bank required", path: ["bank"] });

export async function listPurchaseOrders(req, res, next) {
  try {
    const { PurchaseOrder } = getModels(req.tenantDb);
    ...
    const [items, total] = await Promise.all([
      PurchaseOrder.find(filter).populate("vendor","name").populate("bank","name").sort({ date: -1 }).skip(skip).limit(limit),
...
export async function getPurchaseOrder(req, res, next) {
  try {
    const { PurchaseOrder } = getModels(req.tenantDb);
    const order = await PurchaseOrder.findById(req.params.id).populate("vendor").populate("bank").populate("items.item","name");
...
export async function createPurchaseOrderHandler(req, res, next) {
  try {
    const order = await createPurchaseOrder(req.tenantDb, req.body, { postedBy: req.user.id });
