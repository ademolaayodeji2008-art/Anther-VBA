...
export const createBankAdjustmentSchema = z.object({
  bank: z.string().min(1),
  date: optionalDate(),
  deposit: optionalNumber(z.coerce.number().min(0)),
  withdrawal: optionalNumber(z.coerce.number().min(0)),
  narration: z.string().min(1),
}).refine((d) => (d.deposit ?? 0) > 0 || (d.withdrawal ?? 0) > 0, { message: "Either deposit or withdrawal must be > 0" });

export async function listBankTransactions(req, res, next) {
  try {
    const { BankTransaction } = getModels(req.tenantDb);
    ...
    const [items, total] = await Promise.all([
      BankTransaction.find(filter).populate("bank","name").sort({ date: -1 }).skip(skip).limit(limit),
...
export async function getBankAccountBalance(req, res, next) {
  try {
    ...
    const balance = await getBankBalance(req.tenantDb, req.params.bankId, asOf);
...
export async function createBankAdjustment(req, res, next) {
  try {
    ...
    const txn = await postBankTransaction(req.tenantDb, { bank, type: "BANK_ADJUSTMENT", deposit, withdrawal, date, sourceModule: "Manual", narration, postedBy: req.user.id });
