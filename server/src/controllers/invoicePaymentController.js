...
}).refine((d) => d.method !== "BANK" || !!d.bank, { message: "bank required when method is BANK", path: ["bank"] });

export async function listInvoicePayments(req, res, next) {
  try {
    const { InvoicePayment } = getModels(req.tenantDb);
...
export async function createInvoicePaymentHandler(req, res, next) {
  try {
    const payment = await recordInvoicePayment(req.tenantDb, req.body, { postedBy: req.user.id });
