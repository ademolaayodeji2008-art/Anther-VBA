import { z } from "zod";
import StockAdjustment, { STOCK_ADJUSTMENT_TYPES } from "../models/StockAdjustment.js";
...
export async function createAdjustment(req, res, next) {
  try {
    const adjustment = await postStockAdjustment({ ...req.body, postedBy: req.user.id });
...
export async function reverseAdjustmentHandler(req, res, next) {
  try {
    const adjustment = await reverseStockAdjustment(req.params.id, {
